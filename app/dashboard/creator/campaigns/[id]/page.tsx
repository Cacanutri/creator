...
export default async function CreatorCampaignDetail({ params }: { params: { id: string } }) {
  ...
  if (!campaign || campaign.status !== "open") redirect("/dashboard/creator/campaigns");
    ...
    <div>
      ...
      {myProposal ? (
        <div className="mt-5 rounded-xl border border-zinc-800 bg-zinc-900/20 p-3">
          ...
          <div className="mt-1 text-sm text-zinc-300">Preço: R$ {Number(myProposal.price).toFixed(2)}</div>
